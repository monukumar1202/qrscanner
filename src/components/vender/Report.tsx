...
import { approveRejectProduct, getProductService } from '../../services/apiService';
...
const Report = () => {
  ...
  async function hancleRequestClick(item: any) {
    ...
    if (response.data === 'success') {
      setUpdate('success');
      alert('Delete Request generated successfully');
    }
  }
            ...
            products.map((item: any, index: any) => {
              return (
                <tr key={index}>
                  ...
                  <td><p style={{cursor:'pointer'}} onClick={() => handleClickImage(item.productImage)}>Image</p></td>
                  <td>
                    <span style={{
                      color : (item.status === "Pending"  ? '#FF0000' : '#32a852') }}>{item.status}</span>
                  </td>
                  <td>
                    <button className="btn btn-danger btn-sm bi bi-x rounded-circle" onClick={() => hancleRequestClick(item)} data-toggle="tooltip" data-placement="top" title="Delect Request"></button>
                  </td>
                </tr>
              )
            })
